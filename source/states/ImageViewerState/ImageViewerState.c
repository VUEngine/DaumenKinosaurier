...
void ImageViewerState::enter(void* owner)
{
	...
	// load stage
	GameState::loadStage(this, (StageSpec*)&ImageViewerSt, NULL, true, false);
...
TextureSpec* ImageViewerState::getTexture(uint8 textureNumber)
{
	switch(this->currentAnimation)
	{
		case 0:
...
{
	int8 actualFrame = AnimatedEntity::getActualFrame(this->imageEntity) + 1;
	int numberOfFrames = AnimatedEntity::getNumberOfFrames(this->imageEntity);

	Printing::text(Printing::getInstance(), "00/00", 40, 7, "Number");
	Printing::int32(Printing::getInstance(), actualFrame, (actualFrame < 10) ? 41 : 40, 7, "Number");
	Printing::int32(Printing::getInstance(), numberOfFrames, (numberOfFrames < 10) ? 44 : 43, 7, "Number");
}
